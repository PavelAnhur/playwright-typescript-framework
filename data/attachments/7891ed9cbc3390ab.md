...
- Name: api/buyer/order-operations.spec.ts >> Buyer API -- Order Operations >> should return 403 when accessing another buyers order
- Location: tests/specs/api/buyer/order-operations.spec.ts:83:3
...
```ts
  ...
  8   | 
  9   |   test.beforeEach(async ({ api, authedBuyer }) => {
  10  |     await api.post('_reset');
  11  |     await authedBuyer.delete('cart');
  12  |   });
  13  | 
  14  |   test('should get order history', async ({ api, authedBuyer, createOrder }) => {
  15  |     const products = await api.get('products');
  16  |     const data = await products.json();
  17  |     const testProduct = data.products[6];
  18  |     await createOrder([
  19  |       { productId: testProduct.id, quantity: 1 }
  20  |     ]);
  21  |     const response = await authedBuyer.get('orders');
